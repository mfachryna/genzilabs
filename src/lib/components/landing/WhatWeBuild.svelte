<script lang="ts">
    ...
    const icons = [
        `<svg width="32" height="32" viewBox="0 0 24 24" fill="none" class="text-brand-neon" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M12 8v4"/><path d="M12 16h.01"/></svg>`,
        `<svg width="32" height="32" viewBox="0 0 24 24" fill="none" class="text-brand-neon" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><path d="M12 17h.01"/></svg>`,
        `<svg width="32" height="32" viewBox="0 0 24 24" fill="none" class="text-brand-neon" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 2h4"/><path d="M12 14v-4"/><path d="M4 13a6 6 0 0 1 3-5.94"/><path d="M20 13a6 6 0 0 0-3-5.94"/><path d="M12 14a4 4 0 1 0-7.25-2.25"/><path d="M12 14a4 4 0 1 1 7.25-2.25"/></svg>`
    ];
...
<section class="py-32 bg-dark-bg relative overflow-hidden" bind:this={sectionRef}>
    <div class="container space-y-20">
        ...
        <div class="text-center space-y-6 max-w-3xl mx-auto">
            <span class="text-brand-neon font-mono text-sm uppercase tracking-widest mb-2 block">{$_('whatWeBuild.label')}</span>
            <h2 class="text-4xl md:text-6xl font-bold leading-tight text-white">
                {$_('whatWeBuild.headline')}
            </h2>
        </div>

        <!-- Grid -->
        <div class="grid md:grid-cols-3 gap-6 lg:gap-8">
            {#each $_('whatWeBuild.items') as item, i}
